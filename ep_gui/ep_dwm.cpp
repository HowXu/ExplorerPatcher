...
std::optional<std::ptrdiff_t> locate_udwm_desktop_manager() {
    ...
    auto const dwm_client_startup = reinterpret_cast<uint64_t>(GetProcAddress(udwm_dll, MAKEINTRESOURCEA(101))); // base: MAKEINTRESOURCE
...
std::optional<uint64_t> find_module_base(DWORD pid, std::string_view module_name) noexcept {
    ...
    if (snapshot != INVALID_HANDLE_VALUE) {
        ...
        if (Module32FirstW(snapshot, &entry)) {
            do {
                // 将 std::string_view 转换为 std::wstring_view
                int wLen = MultiByteToWideChar(CP_UTF8, 0, module_name.data(), module_name.length(), NULL, 0);
                if (wLen > 0) {
                    std::wstring wModuleName(wLen, L'\0');
                    MultiByteToWideChar(CP_UTF8, 0, module_name.data(), module_name.length(), &wModuleName[0], wLen);
                    std::wstring_view wModuleNameView = wModuleName;

                    if (wModuleNameView == entry.szModule) {
    ...
    CloseHandle(snapshot); // 确保在没有找到模块时也关闭句柄
    return {}; // 没有找到模块，返回空 optional
}

std::optional<uint64_t> find_module_base_old(DWORD pid, std::string_view module_name) noexcept {
...
int run() try
{
    ...
    if (!enable_privilege(SE_DEBUG_NAME))
        throw std::runtime_error(std::format("Failed to enable SeDebugPrivilege, make sure you are running as admin."));
